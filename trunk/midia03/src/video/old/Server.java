...
public class Server extends JFrame implements ActionListener, Runnable {
  ...
  int remoteRTPPort = 25000; //destination port for RTP packets  (given by the RTSP Client)
...
/* ### MOD 2.1 */
  ...
  //private int RTSP_ID; //ID of the RTSP session
/* ### END MOD 2.1 */
  final static String CRLF = "\r\n";

  /**
   * Deve iniciar o servidor RSTP e 
   * */
  public Server(){
 ...
 // ServerSocket serverSocket;
  ...
  public void run() 
  {
    try{
       ...
       //Get Client IP address
        ...
        System.out.println("Exception on run: " + e.toString());
        this.RTPsocket.close();
    }
  }
  //------------------------
  ...
  public void actionPerformed(ActionEvent e) {
	...
	try {
	  ...
	  senddp = new DatagramPacket(packet_bits, packet_length, ClientIPAddr, remoteRTPPort);
	  ...
	  {
	    System.out.println("Exception caught on actionPerformed: "+ex);
            this.RTPsocket.close();
	    //System.exit(0);/* ### MOD 2.4 */
	  }
  ...
  private int parse_RTSP_request()
  {
      ...
      if (request_type == SETUP)
	{
	  ...
	  remoteRTPPort = Integer.parseInt(tokens.nextToken());
	}
      //else LastLine will be the SessionId line ... do not check for now.
    }
    catch(SocketException ex)
      {
	System.out.println("Conexão encerrada: "+ex);
        end = true;
        return TEARDOWN;
	//System.exit(0);/* ### MOD 2.4 */
      }
    catch(Exception e){
         System.out.println("Fim do video?: "+e);
         this.RTPsocket.close();
    }
  ...
  private void send_RTSP_response()
  {
    try{
      ...
      RTSPBufferedWriter.write("Session: "+(Server.RTSP_ID++)+CRLF);
      RTSPBufferedWriter.flush();
      System.out.println("RTSP Server - Sent response to Client.");
    }
    catch(Exception ex)
      {
	System.out.println("Exception caught on send_rtsp: "+ex);
        RTPsocket.close();
	//System.exit(0);/* ### MOD 2.4 */
      }
  }
